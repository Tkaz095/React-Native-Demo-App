...
export default function MemberEditPage() {
    ...
    const addCert = () => {
        if (newCert.trim() && data && !isLocked) {
            setData({ ...data, certificates: [...data.certificates, newCert.trim()] });
            setNewCert('');
        }
    };
    ...
    const InputField = ({ label, value, onChangeText, multiline = false, keyboardType = 'default' }: any) => (
        <View style={styles.inputGroup}>
            <Text style={styles.label}>{label}</Text>
            <View style={[styles.inputWrapper, isLocked && styles.inputWrapperDisabled]}>
                <TextInput
                    style={[styles.input, multiline && styles.inputMultiline, isLocked && styles.inputDisabled]}
                    value={value}
    ...
    return (
        ...
        >
            ...
            <View style={styles.header}>
                ...
                <TouchableOpacity
                    style={[styles.lockButton, isLocked ? styles.lockButtonActive : styles.lockButtonInactive]}
                    onPress={toggleLock}
                >
                    <Ionicons name={isLocked ? "lock-closed" : "lock-open"} size={20} color={isLocked ? "#B91C1C" : "#059669"} />
                    <Text style={[styles.lockText, isLocked ? { color: '#B91C1C' } : { color: '#059669' }]}>
                        {isLocked ? 'Khóa' : 'Mở'}
                    </Text>
                </TouchableOpacity>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                ...
                <View style={styles.card}>
                    ...
                    <InputField
                        ...
                        onChangeText={(text: string) => setData({ ...data, companyName: text })}
                    />
                    <InputField
                        label="Mã số thuế"
                        value={data.taxCode}
                    ...
                    <InputField
                        ...
                        value={data.industry}
                        onChangeText={(text: string) => setData({ ...data, industry: text })}
                    />
                    ...
                    <View style={styles.row}>
                        <View style={styles.flex1}>
                            <InputField
                                ...
                                onChangeText={(text: string) => setData({ ...data, foundedYear: text })}
                        ...
                        <View style={styles.flex1}>
                            <InputField
                                ...
                                onChangeText={(text: string) => setData({ ...data, employeeSize: text })}
                            />
                        </View>
                    </View>
                    ...
                    <View style={styles.chipsContainer}>
                        {data.services.map((service, index) => (
                            <View key={index} style={styles.chip}>
                                ...
                                {!isLocked && (
                                    <TouchableOpacity onPress={() => removeService(index)} style={styles.chipRemove}>
                    ...
                    <View style={styles.listContainer}>
                        {data.certificates.map((cert, index) => (
                            <View key={index} style={styles.listItem}>
                                ...
                                {!isLocked && (
                                    <TouchableOpacity onPress={() => removeCert(index)} style={styles.itemRemove}>
