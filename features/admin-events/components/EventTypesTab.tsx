...
export default function EventTypesTab({ initialTypes }: Props) {
    ...
    const handleDelete = (id: string, name: string) => {
        Alert.alert('Xác nhận xóa', `Bạn có chắc muốn xóa loại sự kiện "${name}"?`, [
    ...
    const toggleStatus = (id: string) => {
        setEventTypes(eventTypes.map(type =>
            type.id === id ? { ...type, isActive: !(type.isActive !== false) } : type
        ));
    };
        ...
        if (editingType) {
            setEventTypes(eventTypes.map(type =>
                type.id === editingType.id
                    ? { ...type, name: formData.name, description: formData.description, isActive: formData.isActive }
                    : type
            ));
            Alert.alert('Thành công', 'Đã cập nhật loại sự kiện');
        } else {
            const newId = String(Math.max(...eventTypes.map(t => parseInt(t.id) || 0), 0) + 1);
            ...
            <View style={styles.card}>
                ...
                <View style={styles.cardActions}>
                    <TouchableOpacity style={styles.actionBtn} onPress={() => handleEdit(item)}>
                        ...
                        <Text style={[styles.actionBtnText, { color: '#2563EB' }]}>Sửa</Text>
                    </TouchableOpacity>
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                ...
                ListHeaderComponent={
                    <Text style={styles.resultCount}>Hiển thị {filteredEventTypes.length} loại sự kiện</Text>
                }
                ListEmptyComponent={
                ...
                >
                    <View style={styles.modalContent}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>{editingType ? 'Chỉnh sửa loại' : 'Thêm mới loại'}</Text>
                        ...
                        <ScrollView style={styles.modalBody}>
                            <View style={styles.inputGroup}>
                                <Text style={styles.label}>Tên loại sự kiện <Text style={styles.required}>*</Text></Text>
                                <TextInput
                                    style={styles.input}
                                    value={formData.name}
                            ...
                            >
                                <View style={[styles.checkbox, formData.isActive && styles.checkboxChecked]}>
                                    {formData.isActive && <Ionicons name="checkmark" size={14} color="#FFFFFF" />}
                                </View>
                        ...
                        <View style={styles.modalFooter}>
                            ...
                            <TouchableOpacity style={styles.submitBtn} onPress={handleSubmit}>
                                ...
                                <Text style={styles.submitBtnText}>{editingType ? 'Cập nhật' : 'Lưu mới'}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </KeyboardAvoidingView>
