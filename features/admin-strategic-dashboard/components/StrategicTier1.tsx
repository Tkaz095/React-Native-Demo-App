...
function ProgressBar({ value, max, color }: { value: number; max: number; color?: string }) {
    return (
        <View style={shared.progressTrack}>
            <View style={[shared.progressFill, { width: `${(value / max) * 100}%`, backgroundColor: color ?? '#1E3A5F' }]} />
        </View>
    );
}
    ...
    return (
        <Card>
            ...
            <View style={styles.bigValueRow}>
                ...
                <Text style={styles.bigValueSub}>({d.score}/{d.maxScore})</Text>
                    ...
                    <View key={cat.label} style={styles.categoryItem}>
                        <View style={styles.categoryLabelRow}>
                            ...
                            <Text style={styles.categoryValue}>
                                ...
                                <Text style={styles.categoryValueSub}> ({cat.value}/{cat.max})</Text>
                            </Text>
                        </View>
...
function ConnectionRateCard() {
    ...
    return (
        <Card>
            <CardHeader title="Tỷ Lệ Kết Nối Liên Thông" description={d.description} />
            <View style={styles.bigValueRow}>
                <Text style={styles.bigValue}>{d.rate}{d.unit}</Text>
                    ...
                    <View key={step.label}>
                        <View style={styles.funnelRow}>
                            ...
                            <View style={styles.funnelBarWrap}>
                                ...
                                >
                                    <Text style={styles.funnelBarText}>{step.value.toLocaleString()}</Text>
                                </View>
                                {step.percent && (
...
export function StrategicTier1() {
    return (
        <View style={styles.tier}>
            <SectionLabel title="TẦNG 1 – CHỈ SỐ DẪN DẮT" subtitle="(Quan trọng nhất)" />
...
const styles = StyleSheet.create({
    ...
    categoryList: { gap: 14 },
    categoryItem: { gap: 6 },
    categoryLabelRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    ...
    funnelBarWrap: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 },
    funnelBar: {
        height: 32,
        borderRadius: 6,
        alignItems: 'flex-end',
    ...
    conversionNoteText: { fontSize: 12, color: '#065F46', fontWeight: '500', textAlign: 'center' },
});
