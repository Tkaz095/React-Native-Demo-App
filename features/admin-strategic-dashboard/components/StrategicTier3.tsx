...
function ProgressBar({ value, color }: { value: number; color: string }) {
    return (
        <View style={shared.progressTrack}>
            <View style={[shared.progressFill, { width: `${value}%`, backgroundColor: color }]} />
        </View>
    );
}
...
interface DualChartPoint { cost: number; value: number }
...
function DualMiniAreaChart({ data, colorA = '#EF4444', colorB = '#10B981', height = 90, width = 200 }: DualMiniChartProps) {
    ...
    return (
        <Svg width={width} height={height}>
            ...
            <Polyline points={lineFor('cost')} fill="none" stroke={colorA} strokeWidth="2" strokeLinejoin="round" />
            <Path d={pathFor('value')} fill={colorB} fillOpacity={0.1} />
            <Polyline points={lineFor('value')} fill="none" stroke={colorB} strokeWidth="2.5" strokeLinejoin="round" />
        </Svg>
    );
}
                ...
                {d.sources.map((s) => (
                    <View key={s.label} style={styles.sourceItem}>
                        <View style={styles.sourceTitleRow}>
                            ...
                            <Text style={styles.sourceTotal}>{s.total} {s.unit}</Text>
                        </View>
        ...
        <Card>
            ...
            <View style={styles.chartWrap}>
                <View style={{ width: '100%', height: 90 }}>
                    <DualMiniAreaChart data={d.chartData} colorA="#EF4444" colorB="#10B981" height={90} width={320} />
                </View>
            ...
            <View style={styles.summaryNote}>
                ...
                <Text style={styles.summaryText}>
                    Tháng này <Text style={styles.summaryGreen}>giá trị tạo ra</Text> đạt đỉnh, trong khi{' '}
                    <Text style={styles.summaryRed}>chi phí</Text> duy trì ổn định.
                </Text>
            </View>
        </Card>
    );
        ...
        <Card>
            ...
            <View style={styles.donutWrap}>
                <Svg width={110} height={110} viewBox="0 0 100 100">
                    <SvgCircle cx="50" cy="50" r={R} fill="none" stroke="#E5E7EB" strokeWidth="9" />
                    <SvgCircle
                        cx="50" cy="50" r={R}
                        fill="none"
                        stroke="#1E3A5F"
...
const shared = StyleSheet.create({
    ...
    sectionBar: { width: 4, height: 22, borderRadius: 2, backgroundColor: '#1E3A5F' },
...
const styles = StyleSheet.create({
    ...
    barPct: { fontSize: 12, fontWeight: '600', color: '#111827', width: 32, textAlign: 'right' },
    ...
    kpiCost: { backgroundColor: '#FFF1F2', borderLeftColor: '#EF4444', borderWidth: 1, borderColor: '#FEE2E2' },
    kpiValue2: { backgroundColor: '#ECFDF5', borderLeftColor: '#10B981', borderWidth: 1, borderColor: '#A7F3D0' },
    kpiROI: { backgroundColor: '#1E3A5F', borderLeftColor: '#F59E0B', borderWidth: 1, borderColor: '#1E3A5F' },
    kpiTag: { fontSize: 10, fontWeight: '700', color: '#DC2626', letterSpacing: 0.5 },
    kpiTagGreen: { fontSize: 10, fontWeight: '700', color: '#059669', letterSpacing: 0.5 },
    kpiTagWhite: { fontSize: 10, fontWeight: '700', color: '#94A3B8', letterSpacing: 0.5 },
    kpiValue: { fontSize: 17, fontWeight: '800', color: '#111827', marginTop: 2 },
    kpiROIValue: { fontSize: 20, fontWeight: '900', color: '#FFFFFF', marginTop: 2 },
    chartWrap: { gap: 4 },
    ...
    donutWrap: { alignItems: 'center', justifyContent: 'center', position: 'relative', alignSelf: 'center' },
    donutLabel: {
        position: 'absolute',
        alignItems: 'center',
    ...
    healthNoteText: { fontSize: 12, color: '#065F46', fontWeight: '500', flex: 1 },
});
