...
function TrendBadge({ trend, up }: { trend: string; up: boolean }) {
    return (
        <View style={[shared.badge, up ? shared.badgeUp : shared.badgeDown]}>
            <Ionicons name="trending-up" size={11} color={up ? '#059669' : '#DC2626'} />
            <Text style={[shared.badgeText, up ? shared.badgeTextUp : shared.badgeTextDown]}>{trend}</Text>
        </View>
    );
}
...
function ProgressBar({ value, max, color }: { value: number; max: number; color: string }) {
    return (
        <View style={shared.progressTrack}>
            <View style={[shared.progressFill, { width: `${(value / max) * 100}%`, backgroundColor: color }]} />
        </View>
    );
}
...
function MiniAreaChart({ data, color = '#1E3A5F', height = 80, width = 240 }: MiniAreaChartProps) {
        ...
        <Card>
            ...
            <View style={styles.chartWrap}>
                <View style={{ width: '100%', height: 80 }}>
                    <MiniAreaChart data={d.chartData} color="#1E3A5F" height={80} width={320} />
                </View>
                    ...
                    <View key={item.label} style={styles.barRow}>
                        ...
                        <View style={styles.barTrack}>
                            <View
                                style={[
                                    styles.barFill,
                                    { width: `${(item.actual / maxVal) * 100}%`, backgroundColor: item.color },
                                ]}
                            />
                        </View>
                ...
                {d.metrics.map((m) => (
                    <View key={m.label} style={styles.metricItem}>
                        <View style={styles.metricLabelRow}>
                            ...
                            <Text style={styles.metricValue}>{m.value}/{d.maxScore}</Text>
                        </View>
...
function RingMetric({ value, unit, label, color }: { value: number; unit: string; label: string; color: string }) {
    const dashFill = (value / 100) * RING_CIRCUMFERENCE;
    return (
        <View style={styles.ringItem}>
            <Svg width={48} height={48} viewBox="0 0 40 40">
                <SvgCircle cx="20" cy="20" r={RING_R} fill="none" stroke="#E5E7EB" strokeWidth="4" />
                <SvgCircle
            ...
            <Text style={styles.ringValue}>{value}{unit}</Text>
    ...
    return (
        <Card>
            ...
            <View style={styles.ringRow}>
                {d.metrics.map((m) => (
                    <RingMetric key={m.label} value={m.value} unit={m.unit} label={m.label} color={m.color} />
                ))}
            </View>
        ...
        <View style={styles.tier}>
            ...
            <View style={styles.cardList}>
                <View style={styles.row}>
                    <View style={styles.half}><ProjectFormationChart /></View>
                    <View style={styles.half}><ContentEfficiencyChart /></View>
                </View>
                <View style={styles.row}>
                    <View style={styles.half}><InteractionQualityIndex /></View>
                    <View style={styles.half}><NewMemberPerformance /></View>
                </View>
            </View>
        </View>
    );
...
const styles = StyleSheet.create({
    ...
    barValue: { fontSize: 12, fontWeight: '600', color: '#374151', width: 24, textAlign: 'right' },
    metricList: { gap: 12 },
    metricItem: { gap: 6 },
    ...
    ringValue: { fontSize: 13, fontWeight: '700', color: '#111827', marginTop: -2 },
